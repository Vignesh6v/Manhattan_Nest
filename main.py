...
neighborhoodDetails = pd.DataFrame.from_csv("data/nyct2010_neighborhoods_mn.csv")
...
def getscore(userinputs):  
    combine = pd.concat([yelp, recd], axis=1)
    keepfeatures = combine[["Distance", "Crime_Ct", "Complaints", "restaurants", "food", "nightlife"]].applymap(float) + 1
    print keepfeatures.head(n=5)
...
source = ColumnDataSource(data=dict(QI_colmap=ct_colors, ct_x=ct_x, ct_y=ct_y, score=qivals, Price=pricevals, Neighborhood=neighborhoodDetails["NTAName"]))
#print source
hover = HoverTool(
        tooltips=[
            ("Score", "@score"),
...
def update_data(attrname, old, new):
    ...
    source.data = dict(QI_colmap=ct_colors, ct_x=ct_x, ct_y=ct_y, score=qivals, Price=pricevals, Neighborhood=neighborhoodDetails["NTAName"])
